...
Transplant_centers_active_60min_buffer<-list()
Transplant_centers_HIV_60min_buffer<-list()
Transplant_centers_HOPE_60min_buffer<-list()
...
for (year_loop in year_list){
  ...
  for (organ_loop in organ_list){
    ...
    all_center_path<-paste0("cache/Transplant_centers_active_60min_buffer_",organ_loop, "_", year_loop,".RDS")
    if (file.exists(all_center_path)) {
      Transplant_centers_active_60min_buffer[[organ_loop]][[year_loop]] <- 
        readRDS(all_center_path)
    }else {
      
    Transplant_centers_active_60min_buffer[[organ_loop]][[year_loop]]<-
      set_60_min_isochrone_at_7AM(Transplant_centers_active_SF[[organ_loop]][[year_loop]], year_loop)
    saveRDS(Transplant_centers_active_60min_buffer[[organ_loop]][[year_loop]],
            all_center_path)
    }
    ...
    if (file.exists(HIV_center_path)) {
      Transplant_centers_HIV_60min_buffer[[organ_loop]][[year_loop]]<-
        readRDS(HIV_center_path)
    } else {
      Transplant_centers_HIV_60min_buffer[[organ_loop]][[year_loop]]<-
        set_60_min_isochrone_at_7AM(Transplant_centers_HIV_sf[[organ_loop]][[year_loop]], year_loop)
      saveRDS(Transplant_centers_HIV_60min_buffer[[organ_loop]][[year_loop]],
              HIV_center_path)
    }
    ...
    if (file.exists(HOPE_center_path)) {
      Transplant_centers_HOPE_60min_buffer[[organ_loop]][[year_loop]]<-
        readRDS(HOPE_center_path)
    } else {
      Transplant_centers_HOPE_60min_buffer[[organ_loop]][[year_loop]]<-
        set_60_min_isochrone_at_7AM(Transplant_centers_HOPE_sf[[organ_loop]][[year_loop]], year_loop)
      saveRDS(Transplant_centers_HOPE_60min_buffer[[organ_loop]][[year_loop]],
              HOPE_center_path)
    }
  }
}
